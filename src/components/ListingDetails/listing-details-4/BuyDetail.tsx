...
//  Contact: Supabase contacts table + WhatsApp/Email/Call panel
// ============================================================
...
// ─── Phone normaliser (prepend +977 if no country code) ───────
...
const DETAIL_STYLES = `
  ...
  /* ── Breadcrumb ── */
  .fwd-breadcrumb { display: flex; align-items: center; gap: 8px; font-size: 12px; color: var(--c-ink-3); margin-bottom: 22px; flex-wrap: wrap; }
  ...
  .fwd-breadcrumb .current { color: var(--fw-navy); font-weight: 700; max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

  /* ── Banner ── */
  .fwd-banner { padding: 24px 0 20px; border-bottom: 1.5px solid var(--c-rule); margin-bottom: 24px; }
  .fwd-banner__title { font-family: var(--font-display); font-size: clamp(22px, 4vw, 32px); font-weight: 400; color: var(--fw-navy); line-height: 1.2; margin-bottom: 12px; letter-spacing: -0.4px; }
  .fwd-banner__meta { display: flex; align-items: center; flex-wrap: wrap; gap: 7px; }
  .fwd-status-pill { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: var(--radius-pill); font-size: 9.5px; font-weight: 800; letter-spacing: 0.9px; text-transform: uppercase; color: #fff; line-height: 1.6; }
  .fwd-meta-chip { display: inline-flex; align-items: center; gap: 5px; font-size: 12.5px; font-weight: 500; color: var(--c-ink-3); background: var(--c-white); border: 1.5px solid var(--c-rule); padding: 4px 12px; border-radius: var(--radius-pill); }
  .fwd-banner__price-col { text-align: right; }
  @media (max-width: 767px) { .fwd-banner__price-col { text-align: left; margin-top: 16px; } }
  .fwd-banner__price { font-family: var(--font-display); font-size: clamp(26px, 4vw, 34px); font-weight: 400; color: var(--fw-navy); letter-spacing: -0.8px; line-height: 1; }
  .fwd-banner__price sup { font-family: var(--font-body); font-size: 15px; font-weight: 600; color: var(--fw-teal); vertical-align: super; margin-right: 2px; }
  ...
  .fwd-actions { display: flex; align-items: center; gap: 8px; margin-top: 14px; justify-content: flex-end; flex-wrap: wrap; }
  @media (max-width: 767px) { .fwd-actions { justify-content: flex-start; } }
  .fwd-action-share { display: inline-flex; align-items: center; gap: 6px; font-size: 12.5px; font-weight: 700; color: var(--fw-navy); padding: 7px 16px; border-radius: var(--radius-pill); border: 1.5px solid var(--c-rule); background: var(--c-white); text-decoration: none; transition: all 0.18s; margin-right: auto; letter-spacing: 0.2px; cursor: pointer; }
  .fwd-action-share:hover { border-color: var(--fw-teal); color: var(--fw-teal); }
  .fwd-action-icon { width: 36px; height: 36px; border-radius: 50%; border: 1.5px solid var(--c-rule); background: var(--c-white); display: flex; align-items: center; justify-content: center; color: var(--c-ink-3); text-decoration: none; transition: all 0.18s; font-size: 14px; flex-shrink: 0; cursor: pointer; }
  ...
  /* ── Share Modal ── */
  .fwd-modal-overlay { position: fixed; inset: 0; z-index: 1000; background: rgba(15,14,26,0.55); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; padding: 20px; animation: fwd-fade-in 0.18s ease; }
  @keyframes fwd-fade-in { from { opacity: 0; } to { opacity: 1; } }
  .fwd-modal { background: var(--c-white); border-radius: 18px; width: 100%; max-width: 420px; box-shadow: 0 24px 80px rgba(37,32,96,0.22); animation: fwd-slide-up 0.22s cubic-bezier(.22,.9,.36,1); overflow: hidden; }
  @keyframes fwd-slide-up { from { transform: translateY(24px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
  .fwd-modal__header { display: flex; align-items: center; justify-content: space-between; padding: 20px 22px 16px; border-bottom: 1.5px solid var(--c-rule); }
  .fwd-modal__title { font-family: var(--font-display); font-size: 18px; font-weight: 400; color: var(--fw-navy); letter-spacing: -0.2px; }
  .fwd-modal__close { width: 32px; height: 32px; border-radius: 50%; border: 1.5px solid var(--c-rule); background: var(--c-surface); cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 15px; color: var(--c-ink-3); transition: all 0.18s; }
  .fwd-modal__close:hover { background: var(--c-rule); color: var(--c-ink); }
  .fwd-modal__body { padding: 20px 22px 22px; }
  .fwd-share-subtitle { font-size: 12.5px; color: var(--c-ink-3); margin-bottom: 16px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .fwd-share-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 20px; }
  .fwd-share-btn { display: flex; flex-direction: column; align-items: center; gap: 7px; padding: 14px 8px; border-radius: 12px; border: 1.5px solid var(--c-rule); background: var(--c-surface); cursor: pointer; transition: all 0.18s; text-decoration: none; color: var(--c-ink-2); }
  .fwd-share-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(37,32,96,0.1); }
  .fwd-share-btn__icon { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 17px; color: #fff; }
  ...
  .fwd-share-url-row { display: flex; gap: 8px; align-items: center; background: var(--c-surface); border: 1.5px solid var(--c-rule); border-radius: var(--radius-sm); padding: 9px 13px; }
  .fwd-share-url-text { flex: 1; font-size: 12px; color: var(--c-ink-3); font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .fwd-share-copy-btn { background: var(--fw-navy); color: #fff; border: none; cursor: pointer; padding: 6px 13px; border-radius: 6px; font-size: 11.5px; font-weight: 700; font-family: var(--font-body); transition: background 0.18s; white-space: nowrap; flex-shrink: 0; }
  ...
  /* ── Overview bar ── */
  .fwd-overview { display: grid; grid-template-columns: repeat(4, 1fr); background: var(--fw-navy); border-radius: var(--radius-card); overflow: hidden; margin-bottom: 40px; box-shadow: 0 4px 24px rgba(37,32,96,0.18); }
  @media (max-width: 576px) { .fwd-overview { grid-template-columns: repeat(2, 1fr); } }
  .fwd-ov-item { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px 12px; border-right: 1px solid rgba(255,255,255,0.08); text-align: center; position: relative; }
  .fwd-ov-item::after { content: ''; position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); width: 0; height: 2px; background: var(--fw-teal); transition: width 0.3s ease; }
  ...
  .fwd-ov-label { font-size: 9px; color: rgba(255,255,255,0.38); text-transform: uppercase; letter-spacing: 0.9px; margin-bottom: 3px; font-weight: 700; }
  ...
  .fwd-accordion__item { border: 1.5px solid var(--c-rule); border-radius: var(--radius-sm); overflow: hidden; background: var(--c-white); transition: border-color 0.18s; }
  .fwd-accordion__btn { width: 100%; text-align: left; display: flex; align-items: center; justify-content: space-between; padding: 13px 16px; font-size: 13.5px; font-weight: 700; font-family: var(--font-body); color: var(--fw-navy); background: var(--c-surface); border: none; cursor: pointer; transition: background 0.18s, color 0.18s; letter-spacing: 0.1px; }
  ...
  .fwd-accordion__chevron { font-size: 11px; transition: transform 0.22s; color: var(--c-ink-3); width: 16px; height: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  ...
  .fwd-feat-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid var(--c-rule); font-size: 13px; }
  ...
  .fwd-amenity { display: inline-flex; align-items: center; gap: 7px; background: var(--fw-teal-faint); border: 1.5px solid rgba(28,148,164,0.18); border-radius: var(--radius-sm); padding: 7px 13px; font-size: 12.5px; font-weight: 600; color: var(--fw-navy); transition: all 0.18s; }
  ...
  .fwd-nearby-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 13px; background: var(--c-white); border-radius: var(--radius-sm); border: 1.5px solid var(--c-rule); font-size: 13px; transition: border-color 0.18s, background 0.18s; }
  ...
  .fwd-ws-icon { width: 40px; height: 40px; border-radius: 10px; background: var(--fw-teal-faint); border: 1.5px solid rgba(28,148,164,0.18); display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 17px; }
  .fwd-ws-info { flex: 1; }
  ...
  .fwd-ws-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg, var(--fw-teal) 0%, var(--fw-teal-light) 100%); transition: width 0.9s cubic-bezier(.22,.9,.36,1); }
  ...
  .fwd-map-footer { display: flex; align-items: center; gap: 6px; padding: 11px 15px; background: var(--c-white); font-size: 12.5px; color: var(--c-ink-3); border-top: 1.5px solid var(--c-rule); }
  ...
  /* ── Similar cards ── */
  .fwd-similar-card { border-radius: var(--radius-card); overflow: hidden; border: 1.5px solid var(--c-rule); background: var(--c-white); box-shadow: var(--shadow-card); transition: box-shadow 0.25s, transform 0.25s; margin: 0 8px 8px; }
  ...
  .fwd-similar-card__footer { display: flex; justify-content: space-between; align-items: center; padding: 9px 15px 13px; border-top: 1px solid var(--c-rule); }
  .fwd-similar-badge { font-size: 9.5px; font-weight: 800; letter-spacing: 0.6px; color: #fff; padding: 3px 10px; border-radius: var(--radius-pill); text-transform: uppercase; }
  .fwd-similar-arrow { width: 30px; height: 30px; border-radius: 50%; background: var(--fw-navy); color: #fff; display: flex; align-items: center; justify-content: center; text-decoration: none; font-size: 12px; transition: background 0.2s, transform 0.22s; }
  .fwd-similar-arrow:hover { background: var(--fw-teal); transform: rotate(45deg); }

  /* ── Sidebar cards ── */
  .fwd-sidebar-card { background: var(--c-white); border-radius: var(--radius-card); border: 1.5px solid var(--c-rule); box-shadow: var(--shadow-card); padding: 22px; margin-bottom: 16px; }
  .fwd-sidebar-label { font-size: 9px; font-weight: 800; letter-spacing: 1.2px; text-transform: uppercase; color: var(--fw-teal); margin-bottom: 14px; display: block; }
  .fwd-sidebar-price { font-family: var(--font-display); font-size: 28px; font-weight: 400; color: var(--fw-navy); letter-spacing: -0.8px; line-height: 1; margin-bottom: 4px; }
  ...
  .fwd-facts-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid var(--c-rule); font-size: 13px; }
  ...
  .fwd-facts-val--badge { display: inline-flex; align-items: center; background: var(--fw-teal-faint); color: var(--fw-teal-dark); border: 1px solid rgba(28,148,164,0.25); border-radius: var(--radius-pill); padding: 2px 10px; font-size: 11.5px; font-weight: 800; letter-spacing: 0.2px; }

  /* ── Agent sidebar ── */
  .fwd-agent-avatar { width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, var(--fw-teal) 0%, var(--fw-navy) 100%); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; color: #fff; flex-shrink: 0; font-family: var(--font-display); font-weight: 400; }
  ...
  .fwd-channel-btn:hover { transform: translateY(-2px); border-color: transparent; }
  .fwd-channel-btn__icon {
    width: 38px; height: 38px; border-radius: 10px;
    display: flex; align-items: center; justify-content: center;
    font-size: 18px; color: #fff; flex-shrink: 0;
  }
  .fwd-channel-btn__label {
    font-size: 10.5px; font-weight: 700; letter-spacing: 0.3px;
    text-transform: uppercase; color: var(--c-ink-3);
  }
  .fwd-channel-btn--wa .fwd-channel-btn__icon { background: #25D366; }
  .fwd-channel-btn--wa:hover { background: rgba(37,211,102,0.07); border-color: #25D366; box-shadow: 0 4px 16px rgba(37,211,102,0.18); }
  .fwd-channel-btn--wa:hover .fwd-channel-btn__label { color: #1a9e4e; }
  .fwd-channel-btn--call .fwd-channel-btn__icon { background: var(--fw-teal); }
  .fwd-channel-btn--call:hover { background: var(--fw-teal-faint); border-color: var(--fw-teal); box-shadow: 0 4px 16px rgba(28,148,164,0.18); }
  .fwd-channel-btn--call:hover .fwd-channel-btn__label { color: var(--fw-teal-dark); }
  .fwd-channel-btn--email .fwd-channel-btn__icon { background: var(--fw-navy); }
  .fwd-channel-btn--email:hover { background: var(--fw-navy-faint); border-color: var(--fw-navy); box-shadow: 0 4px 16px rgba(37,32,96,0.12); }
  .fwd-channel-btn--email:hover .fwd-channel-btn__label { color: var(--fw-navy); }

  /* ── Contact form ── */
  .fwd-contact-divider {
    display: flex; align-items: center; gap: 10px;
    font-size: 9.5px; font-weight: 800; letter-spacing: 1.2px;
    text-transform: uppercase; color: var(--c-ink-3); margin-bottom: 14px;
  }
  .fwd-contact-divider::before,
  .fwd-contact-divider::after { content: ''; flex: 1; height: 1px; background: var(--c-rule); }
  .fwd-input { width: 100%; padding: 9px 13px; border-radius: var(--radius-sm); border: 1.5px solid var(--c-rule); font-size: 13px; font-family: var(--font-body); color: var(--c-ink); background: var(--c-surface); outline: none; transition: border-color 0.18s, background 0.18s, box-shadow 0.18s; margin-bottom: 9px; appearance: none; }
  .fwd-input:focus { border-color: var(--fw-teal); background: var(--c-white); box-shadow: 0 0 0 3px rgba(28,148,164,0.12); }
  .fwd-textarea { width: 100%; padding: 9px 13px; border-radius: var(--radius-sm); border: 1.5px solid var(--c-rule); font-size: 13px; font-family: var(--font-body); color: var(--c-ink); background: var(--c-surface); outline: none; transition: border-color 0.18s, background 0.18s, box-shadow 0.18s; resize: none; margin-bottom: 11px; }
  .fwd-textarea:focus { border-color: var(--fw-teal); background: var(--c-white); box-shadow: 0 0 0 3px rgba(28,148,164,0.12); }
  .fwd-send-btn { width: 100%; padding: 11px; border-radius: var(--radius-sm); border: none; background: var(--fw-teal); color: #fff; font-size: 13.5px; font-weight: 700; font-family: var(--font-body); cursor: pointer; letter-spacing: 0.3px; transition: background 0.2s; text-transform: uppercase; display: flex; align-items: center; justify-content: center; gap: 8px; }
  .fwd-send-btn:hover:not(:disabled) { background: var(--fw-teal-dark); }
  .fwd-send-btn:disabled { opacity: 0.55; cursor: not-allowed; }
  .fwd-form-error { background: #fff5f5; border: 1.5px solid #fca5a5; border-radius: var(--radius-sm); padding: 9px 13px; font-size: 12.5px; color: #dc2626; margin-bottom: 10px; }
  .fwd-form-success { background: rgba(56,161,105,0.08); border: 1.5px solid rgba(56,161,105,0.3); border-radius: var(--radius-sm); padding: 11px 13px; font-size: 13px; color: #276749; font-weight: 600; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; }
  .fwd-spinner { width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.35); border-top-color: #fff; border-radius: 50%; animation: fwd-spin 0.7s linear infinite; }
  ...
  .fwd-skeleton { background: linear-gradient(90deg, #eeedf6 25%, #e4e3f0 50%, #eeedf6 75%); background-size: 200% 100%; animation: fwd-shimmer 1.4s infinite; border-radius: 8px; }
  ...
  /* ── Mobile sticky CTA ── */
  .fwd-mobile-cta { display: none; position: fixed; bottom: 0; left: 0; right: 0; background: var(--fw-navy); padding: 12px 20px; z-index: 90; box-shadow: 0 -4px 24px rgba(37,32,96,0.18); align-items: center; gap: 12px; }
  @media (max-width: 1199px) { .fwd-mobile-cta { display: flex; } }
  .fwd-mobile-cta__price { font-family: var(--font-display); font-size: 20px; color: #fff; font-weight: 400; letter-spacing: -0.5px; line-height: 1; flex: 1; }
  ...
  .fwd-mobile-cta__btn { background: var(--fw-teal); color: #fff; border: none; cursor: pointer; padding: 10px 20px; border-radius: var(--radius-sm); font-weight: 700; font-size: 13px; font-family: var(--font-body); letter-spacing: 0.3px; transition: background 0.2s; white-space: nowrap; }
  ...
  .fwd-back-btn { display: inline-flex; align-items: center; gap: 7px; padding: 10px 22px; background: var(--fw-navy); color: #fff; border-radius: var(--radius-sm); text-decoration: none; font-weight: 700; font-size: 13.5px; font-family: var(--font-body); transition: background 0.2s; border: none; cursor: pointer; }
  .fwd-back-btn:hover { background: var(--fw-teal); }

  /* ── Toast notification ── */
  .fwd-toast { position: fixed; bottom: 80px; left: 50%; transform: translateX(-50%) translateY(12px); background: var(--fw-navy); color: #fff; padding: 10px 20px; border-radius: var(--radius-pill); font-size: 13px; font-weight: 600; box-shadow: 0 8px 32px rgba(37,32,96,0.25); z-index: 2000; opacity: 0; transition: opacity 0.25s, transform 0.25s; pointer-events: none; white-space: nowrap; }
    ...
    <div className="fwd-nearby-grid">
      {entries.map(([k, v]) => (
        <div key={k} className="fwd-nearby-item">
          ...
          <span className="fwd-nearby-val">{v} km</span>
        </div>
      ))}
    </div>
  );
...
function AgentContactSidebar({
  ...
  const handleSubmit = async () => {
    ...
    if (!msg.trim()) return setError("Please enter a message.");
    ...
    <div className="fwd-sidebar-card" ref={sidebarRef}>
      ...
      {hasAgent && (
        <>
          ...
          {agent!.location && (
            <div className="fwd-agent-row" style={{ marginBottom: "18px" }}>
          ...
          <div className="fwd-contact-channels">
            {phone && (
              ...
              >
                ...
                <span className="fwd-channel-btn__label">WhatsApp</span>
              </a>
            )}
            {phone && (
              <a
              ...
              >
                ...
                <span className="fwd-channel-btn__label">Call</span>
              </a>
            )}
            {agent!.email && (
              <a
              ...
              >
                ...
                <span className="fwd-channel-btn__label">Email</span>
              </a>
            )}
          </div>

          <div className="fwd-contact-divider">or send a message</div>
        </>
      )}
      ...
      {success ? (
        <div className="fwd-form-success">
        ...
        <>
          ...
          />
          {error && <div className="fwd-form-error">{error}</div>}
          <button
          ...
          >
            {submitting ? (
              <>
                <span className="fwd-spinner" /> Sending…
              </>
            ) : (
              <>
